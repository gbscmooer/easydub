...
### 第四轮自主优化 ✅ 完成（2026-09-06 晚，d2a7604 / bec158b）

- [x] **E5 ASR 通道选型对比**（评测体系最后一块）：云端 fish CER 0.0~0.07 vs
      本地 whisper base 0.04~0.35（低 BGM 退化重）→ 默认云端；偏差方向随通道
      相反（fish 偏晚/whisper 偏早），双向平移补偿后双通道偏移 ≤0.04s
- [x] **Web 产品面补齐**：BGM/口型开关透传、历史任务列表；**修复进度回调
      回归**（Web 进度条卡 5%——run_managed 未透传回调，真实 E2E 抓出）

### 第五轮自主优化 ✅ 完成（2026-09-06 晚，b5f5ab4）

- [x] **无声视频健壮闭环**：无音轨视频走零段落正常出片（原来 ffmpeg 直接炸）；
      静音音轨原声直通，两类 e2e 验证
- [x] **可视化 HTML 对齐报告**：每次 run 自动生成 report.<lang>.html
      （动作着色时间轴 + 悬停详情 + 汇总卡），`report --html` 可再生
- [x] **字幕样式透传**：`--subtitle-style` CLI 与 API 字段（管线参数补齐产品面）
